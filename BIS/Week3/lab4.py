import numpy as np
import random
# ---------- PARAMETERS ---------- NUM_ANTS = 20
NUM_ITER = 100
ALPHA = 1 # influence of pheromone
BETA = 5 # influence of heuristic (1/cost)
RHO = 0.5 # evaporation rate
Q = 100 # pheromone deposit factor
# ---------- USER INPUT ---------- n = int(input("Enter number of routers (nodes): "))
...
# ---------- HELPER FUNCTIONS ---------- def path_cost(path): """Compute total cost of a path"""
return sum(cost_matrix[path[i]][path[i+1]] for i in range(len(path)-1))
# ---------- MAIN ACO LOOP ---------- best_path = None
best_cost = float('inf')
